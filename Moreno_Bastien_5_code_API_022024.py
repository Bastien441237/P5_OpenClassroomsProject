...
def encode_text_with_embedding(text, embedding_model):
    # Nettoyer le texte
    ...
    embedded_text = embedding_model([cleaned_text]).numpy()
...
def predict():
    try:
        ...
        # Utiliser la matrice d'embedding pour encoder le texte
        encoded_text = encode_text_with_embedding(data['text'], use_model)
